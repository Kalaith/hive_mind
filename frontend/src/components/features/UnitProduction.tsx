...
const UnitProduction: React.FC = () => {
  const { units, resources, canAfford, spendResources, addUnit } =
    useGameStore();
  ...
  const getUnitCost = (unitType: UnitType) => {
    ...
    return {
      ...
      knowledge: unitType.baseCost.knowledge
        ? Math.floor(unitType.baseCost.knowledge * costMultiplier)
        : 0,
      territory: unitType.baseCost.territory
        ? Math.floor(unitType.baseCost.territory * costMultiplier)
        : 0,
    };
  };
            ...
            >
              ...
              <div className="mb-3">
                ...
                <div className="flex gap-2 text-xs">
                  {Object.entries(unitType.production).map(
                    ([resource, amount]) => (
                      <span
                        key={resource}
                        className={`px-2 py-1 rounded ${amount! > 0 ? "bg-green-600" : "bg-red-600"}`}
                      >
                        {resource}: {amount! > 0 ? "+" : ""}
                        {amount}/s
                      </span>
                ...
                <div className="flex gap-2 text-xs">
                  {Object.entries(cost).map(([resource, amount]) => {
                    ...
                    return (
                      <span
                        key={resource}
                        className={`px-2 py-1 rounded ${hasEnough ? "bg-blue-600" : "bg-red-600"}`}
                      >
